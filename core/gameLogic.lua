...
-- GameHelpers module content
GameLogic.GameHelpers = {}

local characters, state, game, registry, activeFX, Combat

function GameLogic.GameHelpers.init(chars, st, g, reg, fx, comb)
    characters = chars
    state = st
    game = g
    registry = reg
...
function GameLogic.GameHelpers.findCharacterAt(col, row)
...
function GameLogic.GameHelpers.removeCharacter(target)
...
function GameLogic.GameHelpers.handleSelection(clicked)
...
function GameLogic.GameHelpers.performAttack(attacker, defender)
    ...
    if res.type == "attack" then
        if res.result == "hit" then
            ...
            if res.ko then
                ...
                GameLogic.GameHelpers.removeCharacter(res.defender)
            end
