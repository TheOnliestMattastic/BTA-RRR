...
function game.load()
	...
	local stats = {}
	for k, v in pairs(ninjaStats) do
		stats[k] = v
	end
	stats.team = 0
	...
	stats = {}
	for k, v in pairs(gladiatorStats) do
		stats[k] = v
	end
	stats.team = 1
...
function game.update(dt)
	...
	for _, character in ipairs(characters) do
		if character.update then
			pcall(character.update, character, dt)
		end
	end

	-- Update: Active visual effects
	for _, activeEffect in ipairs(activeFX) do
		if activeEffect.fx and activeEffect.fx.anim and activeEffect.fx.anim.update then
			pcall(activeEffect.fx.anim.update, activeEffect.fx.anim, dt)
		end
	end
...
function game.draw()
	...
	if GameUI.actionMenuState.activeButton == 0 then
		map:highlightMovementRange(game.selected, function(col, row)
			return GameHelpers.findCharacterAt(col, row) ~= nil
		end)
	end
	...
	for _, character in ipairs(characters) do
		pcall(function()
			character:draw(map.tileSize, map.offsetX, map.offsetY)
		end)
	end
	...
	for _, activeEffect in ipairs(activeFX) do
		activeEffect.fx.anim:draw(
			activeEffect.fx.image,
			activeEffect.x * map.tileSize + map.offsetX,
...
function game.mousepressed(x, y, button)
	if state.over or inputFocus ~= "mouse" then
		return
	end
	computeScale()
	if button ~= 1 or scale <= 0 then
		return
	end
	...
	if GameHelpers.handleSelection(clicked) then
		return
	end
...
function game.mousereleased(x, y, button)
	if button ~= 1 or inputFocus ~= "mouse" then
		return
	end
	computeScale()
	if scale <= 0 then
...
function game.keyreleased(key)
	if state.over or inputFocus ~= "keyboard" then
		return
	end
	...
	elseif focus == "menu" then
		...
		elseif key == "return" then
			...
			if
				GameUI.actionMenuState.isPressed
				and GameUI.actionMenuState.pressedButton == inputHandler.keyboardFocusButton
			then
